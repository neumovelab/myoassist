...
import dm_control
from myosuite.rl_train.myoassist.analyzer.train_log_handler import TrainLogHandler
...
class GaitEvaluatorReflex(GaitEvaluatorBase):
    def load_parameters(self, folder_name):
        import glob
        """
        Load parameters from the *_Best.txt file in the specified folder.
        
        Parameters:
        - folder_name: Name of the folder within cma_dump to search
        
        Returns:
        - params: Loaded parameters
        - param_file: Full path to the parameter file
        """
        base_path = os.path.join('train_logs', "reflex", folder_name)
        if not os.path.exists(base_path):
            raise ValueError(f"Folder {folder_name} not found in train_logs/reflex directory")
        
        # Find the *_Best.txt file
        param_files = glob.glob(os.path.join(base_path, "*_Best.txt"))
        if not param_files:
            raise ValueError(f"No *_Best.txt file found in {folder_name}")
        
        # Use the first (and should be only) Best file found
        param_file = param_files[0]
        params = np.loadtxt(param_file)
        
        print(f"Loaded parameters from: {os.path.basename(param_file)}")
        print(f"Parameter vector size: {len(params)}")
        return params, param_file
    def __init__(self, folder_name:str):
        self.params, self.param_file = self.load_parameters(folder_name)
        
    def initialize_env(self):
        n_points = 5
        import myosuite.envs.myoassist.reflex.Reflex_EXO_Interface_11mus as Reflex_EXO_Interface_11mus
        TestEnv = Reflex_EXO_Interface_11mus.MyoLegReflex_EXO(
            sim_time=20,
            mode='2D',
            init_pose='walk',
            ...
            slope_deg=0,
            delayed=False,
            exo_bool=True,  # Always true for N-point analysis
            n_points=n_points,  # Using the same n_points as in sorting
            use_legacy_spline=False,  # Using new N-point system
            max_torque=50,
            model_path=os.path.join("myosuite","simhive","myoassist_sim", "gait14dof26musc_cvt3_Right_2D_DEPHY.xml")
            # model_path=os.path.join("myosuite","simhive","myoassist_sim", "gait14dof26musc_cvt3_Right_2D_DEPHY.xml")
        )
        TestEnv.reset()
        pose_valid = TestEnv.check_pose_validity()
        spline_valid_R = TestEnv.ExoCtrl_R.check_spline_validity()
        spline_valid_L = TestEnv.ExoCtrl_L.check_spline_validity()
        print("\nParameter Validation:")
        print(f"Pose check: {pose_valid}")
        print(f"Right controller check: {spline_valid_R}")
        print(f"Left controller check: {spline_valid_L}")

        # Setup rendering configuration
        TestEnv.env.unwrapped.sim.renderer.render_offscreen(
            camera_id=1,
            width=1920,
            height=1080
        )
        TestEnv.env.sim.renderer._scene_option.flags[0] = 0  # Remove convex hull
        TestEnv.env.sim.renderer._scene_option.flags[4] = 0  # Remove actuator visualization

        print("\nEnvironment initialized and ready for simulation")

        self.env_wrapper = TestEnv
        self.env = self.env_wrapper.env
    def evaluate(self, result_dir:str, file_name:str) -> str:
        gait_data = GaitData()
        timesteps = int(5 / self.env.dt)
        for time_step in range(timesteps):
            # Run simulation step and collect data
            _, _, is_done = self.env_wrapper.run_reflex_step_Cost()
            gait_data.add_data(mj_model=self.env.sim.model, mj_data=self.env.sim.data, printing=True if time_step == 0 else False)
            

            if is_done:
                print(f"Simulation stopped at step {time_step}")
                break
        gait_data.print_brief_data()

        gait_data_path = os.path.join("train_logs", "reflex", result_dir, file_name)
        if not os.path.exists(os.path.dirname(gait_data_path)):
            print(f"Creating directory {os.path.dirname(gait_data_path)}")
            os.makedirs(os.path.dirname(gait_data_path))
        gait_data.save_json_data(gait_data_path)
        gait_data_read = GaitData()
        gait_data_read.read_json_data(gait_data_path)
        # print("==============================READ DATA==================================")
