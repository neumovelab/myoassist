...
class GaitAnalyzer:
    ...
    def get_gait_segment_index(self, *, is_right_foot_based: bool):
        ...
        for idx, (primary_foot, primary_toes) in enumerate(
            zip(
                [v[0] for v in sensor_data[f"{primary_char}_foot"]["data"]],
                [v[0] for v in sensor_data[f"{primary_char}_toes"]["data"]],
            )
        ):
            if idx > 0:
                prev_primary_combined = (
                    sensor_data[f"{primary_char}_foot"]["data"][idx - 1][0]
    ...
    def joint_angle_by_velocity(self, *, result_dir: str):
        ...
        fig.savefig(os.path.join(result_dir, "joint_angle_cmap_by_velocity.png"))

        if self.show_plot:
            plt.show()
        plt.close()

    def plot_exo_segmented_data(
        self,
        *,
        result_dir,
    ) -> None:
        ...
        axes[0].axvline(
            toe_off_l,
            color=self.toe_off_color,
            linestyle=self.toe_off_linestyle,
            linewidth=self.toe_off_linewidth,
            alpha=self.toe_off_alpha,
        )
        axes[1].axvline(
            toe_off_r,
            color=self.toe_off_color,
        ...
        fig_mean_std, axes_mean_std = plt.subplots(
            1, 2, figsize=(3.5 * self.fig_size_multiplier, 2 * self.fig_size_multiplier), dpi=self.dpi
        )
    ...
    def plot_segmented_kinematics_result(
        self,
        *,
        result_dir,
    ) -> None:
        ...
        for joint, data in joint_data_mapped_degree.items():
            ...
            ax.fill_between(
                x_mapped, mean_data_degree - std_data_degree, mean_data_degree + std_data_degree, color="gray", alpha=0.5
            )
    ...
    def plot_left_right_comparison(
        ...
        *,
        ...
        for joint, data in joint_data_mapped.items():
            ...
            ax.plot(
                x_mapped, mean_data_degree, color=line_color, linestyle=line_style, label="Right" if "_r" in joint else "Left"
            )
            ax.fill_between(
                x_mapped, mean_data_degree - std_data_degree, mean_data_degree + std_data_degree, color=fill_color, alpha=0.5
            )
        ...
        for idx in range(3):
            ...
            axes2[idx].axvline(
                toe_off_r, color=self.toe_off_color, linestyle="-", linewidth=self.toe_off_linewidth, alpha=self.toe_off_alpha
            )
    ...
    def plot_contact_data(
        ...
        *,
        ...
        fig, axes = plt.subplots(
            len(geom_pairs), 1, figsize=(4 * self.fig_size_multiplier, 3 * self.fig_size_multiplier), dpi=self.dpi
        )
    ...
    def plot_segmented_muscle_data(self, *, result_dir, is_plot_right: bool):
        ...
        actuator_names = [
            actuator_name
            for actuator_name in self.gait_data.series_data["actuator_data"].keys()
            if actuator_name[-2:] in post_fix and "Exo" not in actuator_name
        ]
        actuator_names.sort()
        actuator_names += [
            actuator_name
            for actuator_name in self.gait_data.series_data["actuator_data"].keys()
            if actuator_name[-2:] in post_fix and "Exo" in actuator_name
        ]
                ...
                for idx, (start_idx, toe_off_idx, end_idx) in enumerate(gait_segment_index):
                    muscle_data = {
                        ...
                        "ctrl": np.abs(
                            np.interp(
                                x_mapped,
                                np.linspace(0, 100, num=len(actuator_data["ctrl"][start_idx:end_idx])),
        ...
        fig, axes = plt.subplots(
            actuator_num,
            1,
            figsize=(plot_width * self.fig_size_multiplier, plot_height * self.fig_size_multiplier),
            dpi=self.dpi,
        )
        actuator_index = 0
