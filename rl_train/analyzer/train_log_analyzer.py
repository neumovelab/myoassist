...
class TrainLogAnalyzer:
    ...
    def plot_reward_dict(self, *, result_dir: str, show_plot: bool, mult_weights: bool = False):
        ...
        for key in valid_keys:
            try:
                reward_dict_data[key] = [
                    log_data.average_reward_dict_per_episode.get(key, 0.0) for log_data in modified_log_datas
                ]
        ...
        if mult_weights:
            ...
            for key in list(reward_dict_data.keys()):
                if key in reward_weights:
                    reward_dict_data[key] = [
                        value * weight for value, weight in zip(reward_dict_data[key], reward_weights[key])
                    ]
        ...
        for idx, (key, data) in enumerate(negative_data.items()):
            ax.fill_between(
                time_steps,
                bottom,
                [b + d for b, d in zip(bottom, data)],
                label=key,
                color=color_map(idx / len(color_keys)),
                alpha=0.5,
            )
        ...
        fig.savefig(
            os.path.join(
                result_dir,
                f"average_reward_dict_proportion_per_episode_{'mult_weights' if mult_weights else 'no_mult_weights'}.png",
            )
        )
        if show_plot:
            plt.figure(fig.number)
    ...
    def plot_imitation_reward_weights(self, *, result_dir: str):
        time_steps = [log_data.num_timesteps for log_data in self._train_log_handler.log_datas]
        reward_datas = {
            key: [data.reward_accumulate[key] for data in self._train_log_handler.log_datas]
            for key in self._train_log_handler.log_datas[0].reward_accumulate.keys()
        }
        reward_weights_datas = {
            key: [data.reward_weights[key] for data in self._train_log_handler.log_datas]
            for key in self._train_log_handler.log_datas[0].reward_weights.keys()
        }
        # print(reward_datas)
