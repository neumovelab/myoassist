...
class BaseCustomActorCriticPolicy(BasePolicy):
    def __init__(
        ...
        **kwargs,
        ...
        custom_policy_params = DictionableDataclass.create(self._get_custom_policy_type(), custom_policy_params_dict)
        ...
        self.policy_network = self._build_policy_network(observation_space, action_space, custom_policy_params)

        self.action_dist = DiagGaussianDistribution(action_space)

        self.log_std = nn.Parameter(
            th.ones(self.action_space.shape[0], device=self.device) * custom_policy_params.log_std_init, requires_grad=True
        )
