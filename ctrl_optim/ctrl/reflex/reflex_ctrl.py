...
class MyoLocoCtrl(object):
    ...
    def reset_delay_buffers(self, initial_values, init_pose, initial_musc_ctrl, updateFlag=False):
        ...
        for delay_item in self.sens_delay.keys():
            ...
            [
                temp_queue.append(initial_values["body"][item])
                for i in range(int(np.round(self.sens_delay[delay_item] / self.timestep_size)))
            ]
        ...
        for leg in ["r_leg", "l_leg"]:
            ...
            for delay_item in self.sens_leg_delay.keys():
                ...
                [
                    temp_queue.append(initial_values[leg][item])
                    for i in range(int(np.round(self.sens_leg_delay[delay_item] / self.timestep_size)))
                ]
            ...
            for delay_item in self.out_leg_delay.keys():
                ...
                if delay_item in ["alpha_tgt"]:
                    ...
                    [
                        temp_queue.append(init_alpha)
                        for i in range(int(np.round(self.out_leg_delay[delay_item] / self.timestep_size)))
                    ]
                ...
                if delay_item in ["alpha_tgt_f"] and self.control_dimension == 3:
                    ...
                    alpha_tgt_global_f = (
                        self.cp[leg]["alpha_0_f"]
                        - (sign_frontral * self.cp[leg]["C_d_f"] * D_L_f)
                        - (sign_frontral * self.cp[leg]["C_v_f"] * V_L_f)
                    )
                    alpha_tgt_f = alpha_tgt_global_f - (sign_frontral * initial_values["body"]["theta_f"])

                    [
                        temp_queue.append(alpha_tgt_f)
                        for i in range(int(np.round(self.out_leg_delay[delay_item] / self.timestep_size)))
                    ]
    ...
    def update_sensor_buffer(self, sens_data):
        ...
        for leg in ["r_leg", "l_leg"]:
            D_L = (
                self.sens_delay_data["body"]["supra_pelvis_pos"][0][0]
                - self.sens_delay_data[leg]["supra_talus_contra_pos"][0][0]
            )
            V_L = (
                self.sens_delay_data["body"]["supra_pelvis_vel"][0][0]
                - self.sens_delay_data[leg]["supra_talus_contra_vel"][0][0]
            )
            ...
            if self.control_dimension == 3:
                D_L_f = (
                    self.sens_delay_data["body"]["supra_pelvis_pos"][0][1]
                    - self.sens_delay_data[leg]["supra_talus_contra_pos"][0][1]
                )
                V_L_f = (
                    self.sens_delay_data["body"]["supra_pelvis_vel"][0][1]
                    - self.sens_delay_data[leg]["supra_talus_contra_vel"][0][1]
                )
    ...
    def update(self, sensor_data):
        ...
        for s_leg, s_legc in leg_vec:
            out_stim, contra_afferent = self.spinal_control_leg(
                s_leg, self.output_c_aff_delay_data[s_legc], self.spinal_control_phase[s_legc]
            )
    ...
    def update_supraspinal_control(self):
        ...
        for s_leg in ["r_leg", "l_leg"]:
            ...
            temp_alpha[s_leg]["alpha"] = (
                self.sens_delay_data[s_leg]["supra_phi_hip"][0] - 0.5 * self.sens_delay_data[s_leg]["supra_phi_knee"][0]
            )
            temp_alpha[s_leg]["dalpha"] = (
                self.sens_delay_data[s_leg]["supra_dphi_hip"][0] - 0.5 * self.sens_delay_data[s_leg]["supra_dphi_knee"][0]
            )  # Hip flexion vel (-), Knee flexion vel (-)  Only for dalpha calculations

            if self.control_dimension == 3:
                temp_alpha[s_leg]["alpha_f"] = self.sens_delay_data[s_leg]["supra_phi_hip_add"][
                    0
            ...
            self.supraspinal_command[s_leg]["hip_tgt"] = (
                self.supraspinal_command[s_leg]["alpha_tgt"] + 0.5 * cp[s_leg]["knee_tgt"]
            )
        ...
        if self.sens_delay_data["r_leg"]["supra_contact_ipsi"][0] and self.sens_delay_data["l_leg"]["supra_contact_ipsi"][0]:
            if self.control_dimension == 3:
                r_delta_alpha = np.sqrt(
                    (temp_alpha["r_leg"]["alpha"] - self.supraspinal_command["r_leg"]["alpha_tgt"]) ** 2
                    + (temp_alpha["r_leg"]["alpha_f"] - self.supraspinal_command["r_leg"]["alpha_tgt_f"]) ** 2
                )

                l_delta_alpha = np.sqrt(
                    (temp_alpha["l_leg"]["alpha"] - self.supraspinal_command["l_leg"]["alpha_tgt"]) ** 2
    ...
    def update_spinal_phases(self, s_leg):
        ...
        temp_alpha = {}
        temp_alpha["alpha"] = (
            self.sens_delay_data[s_leg]["module_phi_hip"][0] - 0.5 * self.sens_delay_data[s_leg]["module_phi_knee"][0]
        )
        temp_alpha["dalpha"] = (
            self.sens_delay_data[s_leg]["module_dphi_hip"][0] - 0.5 * self.sens_delay_data[s_leg]["module_dphi_knee"][0]
        )  # Hip flexion vel (-), Knee flexion vel (-)  Only for dalpha calculations
    ...
    def spinal_control_leg(self, s_leg, c_afferent, c_spinal):
        ...
        S_GLU_1 = (ph_st_st + ph_st_sw0 * (np.clip(1 - (cp["Tr_St_sup"] * s_l["module_load_contra"][0]), 0, 1))) * np.maximum(
            cp["1_GLU_FG"] * s_l["module_F_GLU"][0], 0
        )

        S_VAS_1 = (ph_st_st + ph_st_sw0 * (np.clip(1 - (cp["Tr_St_sup"] * s_l["module_load_contra"][0]), 0, 1))) * np.maximum(
            cp["1_VAS_FG"] * s_l["module_F_VAS"][0], 0
        )

        S_SOL_1 = (ph_st) * np.maximum(cp["1_SOL_FG"] * s_l["module_F_SOL"][0], 0)

        S_FDL_1 = (ph_st) * np.maximum(cp["1_FDL_FG"] * s_l["module_F_FDL"][0], 0)
        ...
        S_HAM_2 = (ph_st_st + ph_st_sw0 * (np.clip(1 - (cp["Tr_St_sup"] * s_l["module_load_contra"][0]), 0, 1))) * np.maximum(
            cp["2_HAM_FG"] * s_l["module_F_HAM"][0], 0
        )

        S_VAS_2 = (
            -1
            * (ph_st_st + ph_st_sw0 * (np.clip(1 - (cp["Tr_St_sup"] * s_l["module_load_contra"][0]), 0, 1)))
            * np.maximum(cp["2_VAS_BFSH_PG"] * (s_l["module_phi_knee"][0] - knee_off_st), 0)
        )
        ...
        S_GAS_2 = (ph_st) * np.maximum(cp["2_GAS_FG"] * s_l["module_F_GAS"][0], 0)

        # ----- Module 3 -----
        S_HFL_3 = (
            ph_st
            * s_l["module_load_ipsi"][0]
            * np.maximum(-1 * (cp["3_HFL_Th"] * (theta - cp["theta_tgt"])) - (cp["3_HFL_d_Th"] * dtheta), 0)
        )

        S_GLU_3_interim = (
            ph_st * s_l["module_load_ipsi"][0] * ((cp["3_GLU_Th"] * (theta - cp["theta_tgt"])) + (cp["3_GLU_d_Th"] * dtheta))
        )
        ...
        S_HFL_4 = c_spinal["ph_sw"] * (
            cp["4_HFL_C_GLU_PG"] * c_afferent["GLU"][0] + cp["4_HFL_C_HAM_PG"] * c_afferent["HAM"][0]
        )
        ...
        S_TA_5 = np.maximum(cp["5_TA_PG"] * (s_l["module_phi_ankle"][0] - ankle_tgt), 0)
        S_TA_5_st = -1 * ph_st * (cp["5_TA_SOL_FG"] * np.maximum(s_l["module_F_SOL"][0], 0))

        S_EDL_5 = np.maximum(cp["5_EDL_PG"] * (s_l["module_phi_mtp"][0] - mtp_tgt), 0)
        S_EDL_5_st = -1 * ph_st * (cp["5_EDL_FDL_FG"] * np.maximum(s_l["module_F_FDL"][0], 0))
        ...
        S_HFL_6 = (ph_st_sw0 * (cp["Tr_Sw"] * s_l["module_load_contra"][0]) + ph_sw) * np.maximum(
            (cp["6_HFL_RF_PG"] * (temp_alpha["alpha"] - alpha_tgt)) + (cp["6_HFL_RF_VG"] * temp_alpha["dalpha"]), 0
        )

        S_GLU_6 = (ph_st_sw0 * (cp["Tr_Sw"] * s_l["module_load_contra"][0]) + ph_sw) * np.maximum(
            (-1 * cp["6_GLU_HAM_PG"] * (temp_alpha["alpha"] - alpha_tgt)) - (cp["6_GLU_HAM_VG"] * temp_alpha["dalpha"]), 0
        )
        ...
        # ----- Module 8 -----
        S_RF_8 = ph_sw_sw_2 * np.maximum(
            -1 * (cp["8_RF_VG"] * s_l["module_dphi_knee"][0]), 0
        )  # Want to activate when knee is flexing
        S_BFSH_8 = ph_sw_sw_2 * (
            np.maximum(
                cp["8_BFSH_VG"] * (s_l["module_dphi_knee"][0]), 0
            )  # Flex if it is positive for damping (180 deg is extended knee)
            * np.maximum(
                cp["8_BFSH_PG"] * (temp_alpha["alpha"] - alpha_tgt), 0
            )  # Conversion factor unknown so use a optimizable gain for it
        )
        ...
        if self.control_dimension == 3:
            ...
            S_HAB_1 = (
                ph_st_st + ph_st_sw0 * (np.clip(1 - (cp["Tr_St_sup"] * s_l["module_load_contra"][0]), 0, 1))
            ) * np.maximum(cp["1_HAB_FG"] * s_l["module_F_HAB"][0], 0)
            S_HAB_3 = (
                ph_st
                * s_l["module_load_ipsi"][0]
                * np.maximum(
            ...
            S_HAB_6 = (ph_st_sw0 * (cp["Tr_Sw"] * s_l["module_load_contra"][0]) + ph_sw) * np.maximum(
                cp["6_HAB_PG"] * (alpha_f - alpha_tgt_f), 0
            )
            ...
            S_HAD_6 = (ph_st_sw0 * (cp["Tr_Sw"] * s_l["module_load_contra"][0]) + ph_sw) * np.maximum(
                -1 * cp["6_HAD_PG"] * (alpha_f - alpha_tgt_f), 0
            )
    ...
    def set_control_params_leg(self, s_leg, params):
        ...
        cp["knee_sw_tgt"] = (
            params[cp_map["knee_sw_tgt"]] * 20 * np.pi / 180 + 100 * np.pi / 180
        )  # Min 120 angle during knee swing
        ...
        )  # Min 160 deg, during compliant leg
