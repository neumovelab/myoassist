...
class myoLeg_reflex(object):
    ...
    def __init__(
        ...
        # Slope (and other terrain) now comes through the compose pipeline as a
        # ``terrain`` config -- the model is composed and pre-seated on the tilted
        # plane by compose.  ``slope_deg`` is derived metadata (the course grade)
        # for the eval camera / cost / display; it no longer drives geometry here.
        ...
        self.env = ReflexEnvV0(
            model_path=model_xml, normalize_act=False, joint_random_range=(0, 0), frame_skip=self.frame_skip, seed=seed
        )
    ...
    def get_sensor_data(self):
        ...
        self.SENSOR_DATA["body"]["dtheta"] = (
            -1 * torso_euler_vel[1]
        )  # velocity about z-axis (z-axis points to the right of the model), forward (+)
        ...
        x_local, y_local = self.rotate_frame(
            world_com_xpos[0], world_com_xpos[1], -1 * temp_pelvis_euler[2]
        )  # Yaw, Left (+) Right (-)
        dx_local, dy_local = self.rotate_frame(world_com_xvel[0], world_com_xvel[1], -1 * temp_pelvis_euler[2])

        self.SENSOR_DATA["body"]["pelvis_pos"] = np.array(
            [x_local, y_local]
        )  # Local coord (+ direction) [(Forward), (Leftward)]
        self.SENSOR_DATA["body"]["pelvis_vel"] = np.array(
            [dx_local, dy_local]
        )  # Local coord (+ direction) [(Forward), (Leftward)]
        self.SENSOR_DATA["body"]["pelvis_tilt"] = pelvis_tilt  # Forward tilt (+) after conversion
        self.SENSOR_DATA["body"]["pelvis_tilt_vel"] = (
            pelvis_tilt_vel  # velocity about z-axis (z-axis points to the right of the model), forward (+)
        )
        ...
        for s_leg, s_legc in zip(["r_leg", "l_leg"], ["l_leg", "r_leg"]):
            ...
            self.SENSOR_DATA[s_leg]["talus_contra_pos"] = np.array([tal_x_local, tal_y_local])
            self.SENSOR_DATA[s_leg]["talus_contra_vel"] = np.array([tal_dx_local, tal_dy_local])
            ...
            self.SENSOR_DATA[s_leg]["dalpha"] = (
                -1 * self.SENSOR_DATA[s_leg]["dphi_hip"] - 0.5 * self.SENSOR_DATA[s_leg]["dphi_knee"]
            )  # Hip flexion vel (-), Knee flexion vel (-)  Only for dalpha calculations
            # if self.mode == '3D':
            ...
            if self.mode == "3D":
                self.SENSOR_DATA[s_leg]["phi_hip_add"] = (
                    self.env.sim.data.joint(f"hip_adduction_{s_leg[0]}").qpos[0].copy() + 0.5 * np.pi
                )  # Inwards (Add, +), Outwards (Abd, -), for alpha_f
                self.SENSOR_DATA[s_leg]["phi_hip_rot"] = (
                    self.env.sim.data.joint(f"hip_rotation_{s_leg[0]}").qpos[0].copy()
                )  # Inwards (Rot, +), Outwards (Rot, -), for alpha_rot
                self.SENSOR_DATA[s_leg]["dphi_hip_rot"] = (
                    self.env.sim.data.joint(f"hip_rotation_{s_leg[0]}").qvel[0].copy()
                )  # Inwards (Rot, +), Outwards (Rot, -), for alpha_rot
            else:
    ...
    def collectCost(self):
        ...
        cost_dict["pelvis_dist"] = np.array(
            (self.env.sim.data.body("pelvis").xpos[0].copy() - 0, np.abs(self.env.sim.data.body("pelvis").xpos[1].copy() - 0))
        )
        ...
        cost_dict["pelvis"]["theta_tgt"] = self.cp["r_leg"][
            "theta_tgt"
        ]  # Get theta_tgt from ReflexController, any leg will do, since they are the same
        ...
        cost_dict["torso"]["dpitch"] = (
            -1 * torso_euler_vel[1]
        )  # velocity about z-axis (z-axis points to the right of the model), forward (+)
        cost_dict["torso"]["roll_cost"] = torso_euler[0]  # Upright is 0
        cost_dict["torso"]["yaw_cost"] = torso_euler[
            2
        ]  # Forward is 0 (Dependent on current initial pose. Yaw is with ref to world frame)

        cost_dict["body"] = {}
        cost_dict["body"]["head_theta"] = self.get_intrinsic_EulerXYZ(self.env.sim.data.body("head").xquat.copy())[
            1
        ]  # Roll, Pitch, Yaw (Head tilt forward at rest: (+)11.4 deg (0.2 rad))
        ...
        for s_leg in ["r_leg", "l_leg"]:
            ...
            if self.mode == "3D":
                cost_dict[s_leg]["joint"]["hip_add"] = (
                    self.env.sim.data.joint(f"hip_adduction_{s_leg[0]}").qpos[0].copy()
                ) + 0.5 * np.pi
    ...
    def update_footstep(self):
        ...
        r_contact = (
            True
            if (self.env.sim.data.sensor("r_foot").data[0].copy()) > 0.1 * (np.sum(self.env.sim.model.body_mass) * 9.8)
            else False
        )
        l_contact = (
            True
            if (self.env.sim.data.sensor("l_foot").data[0].copy()) > 0.1 * (np.sum(self.env.sim.model.body_mass) * 9.8)
            else False
        )
    ...
    def update_init_pose_param_cmaes(self, jnt_params):
        ...
        if self.init_pose == "walk_left":
            self.JNT_OPTIM["joint_angles"]["hip_flexion_r"] = jnt_params[self.pose_map["hip_flexion_r"]] * 5 * np.pi / 180 + (
                -15 * np.pi / 180
            )
            self.JNT_OPTIM["joint_angles"]["hip_flexion_l"] = jnt_params[self.pose_map["hip_flexion_l"]] * 5 * np.pi / 180 + (
                20 * np.pi / 180
            )
            self.JNT_OPTIM["joint_angles"]["knee_angle_r"] = jnt_params[self.pose_map["knee_angle_r"]] * 5 * np.pi / 180 + (
                -30 * np.pi / 180
            )
            self.JNT_OPTIM["joint_angles"]["knee_angle_l"] = jnt_params[self.pose_map["knee_angle_l"]] * 5 * np.pi / 180 + (
                -10 * np.pi / 180
            )
            self.JNT_OPTIM["joint_angles"]["ankle_angle_r"] = jnt_params[self.pose_map["ankle_angle_r"]] * 5 * np.pi / 180 + (
                -5 * np.pi / 180
            )
            self.JNT_OPTIM["joint_angles"]["ankle_angle_l"] = jnt_params[self.pose_map["ankle_angle_l"]] * 5 * np.pi / 180 + (
                -8 * np.pi / 180
            )
        ...
        if self.mode == "3D":
            self.JNT_OPTIM["joint_angles"]["hip_adduction_r"] = jnt_params[
                self.pose_map["hip_adduction_r"]
            ] * 5 * np.pi / 180 + (-5 * np.pi / 180)
            self.JNT_OPTIM["joint_angles"]["hip_adduction_l"] = jnt_params[
                self.pose_map["hip_adduction_l"]
            ] * 5 * np.pi / 180 + (-5 * np.pi / 180)
            self.JNT_OPTIM["joint_angles"]["hip_rotation_r"] = jnt_params[self.pose_map["hip_rotation_r"]] * 5 * np.pi / 180 + (
                -5 * np.pi / 180
            )
            self.JNT_OPTIM["joint_angles"]["hip_rotation_l"] = jnt_params[self.pose_map["hip_rotation_l"]] * 5 * np.pi / 180 + (
                -5 * np.pi / 180
            )

        if self.mode == "2D":
            self.JNT_OPTIM["joint_angles"]["pelvis_tilt"] = jnt_params[self.pose_map["pelvis_tilt"]] * 1 * np.pi / 180 + (
                -16 * np.pi / 180
    ...
    def _set_muscle_groups(self):
        ...
        temp_L0 = (self.env.sim.model.actuator_lengthrange[:, 1] - self.env.sim.model.actuator_lengthrange[:, 0]) / (
            self.env.sim.model.actuator_gainprm[:, 1] - self.env.sim.model.actuator_gainprm[:, 0]
        )
    ...
    def get_full_plot_data(self):
        ...
        plot_data["body"]["dtheta"] = (
            -1 * pelvis_euler_vel[1][2]
        )  # velocity about z-axis (z-axis points to the right of the model), forward (-)
        ...
        for s_leg, s_legc in zip(["r_leg", "l_leg"], ["l_leg", "r_leg"]):
            ...
            plot_data[s_leg]["joint_torque"]["hip"] = (
                self.env.sim.data.joint(f"hip_flexion_{s_leg[0]}").qfrc_constraint[0].copy()
                + self.env.sim.data.joint(f"hip_flexion_{s_leg[0]}").qfrc_smooth[0].copy()
            )
            plot_data[s_leg]["joint_torque"]["knee"] = (
                self.env.sim.data.joint(f"knee_angle_{s_leg[0]}").qfrc_constraint[0].copy()
                + self.env.sim.data.joint(f"knee_angle_{s_leg[0]}").qfrc_smooth[0].copy()
            )
            plot_data[s_leg]["joint_torque"]["ankle"] = (
                self.env.sim.data.joint(f"ankle_angle_{s_leg[0]}").qfrc_constraint[0].copy()
                + self.env.sim.data.joint(f"ankle_angle_{s_leg[0]}").qfrc_smooth[0].copy()
            )

            # Check sign - BODY FRAME ALPHA
            plot_data[s_leg]["alpha"] = plot_data[s_leg]["joint"]["hip"] - 0.5 * plot_data[s_leg]["joint"]["knee"]
            plot_data[s_leg]["dalpha"] = (
                -1 * plot_data[s_leg]["d_joint"]["hip"] - 0.5 * plot_data[s_leg]["d_joint"]["knee"]
            )  # Hip Flexion Vel (-) Only for dalpha calculations

            if self.mode == "3D":
                plot_data[s_leg]["alpha_f"] = (
                    self.env.sim.data.joint(f"hip_adduction_{s_leg[0]}").qpos[0].copy()
                ) + 0.5 * np.pi
            ...
            for MUS in temp_mus:
                plot_data[s_leg][MUS] = {}
                plot_data[s_leg][MUS]["f"] = -1 * (
                    temp_mus_force[self.muscles_dict[s_leg][MUS]] / (self.muscle_Fmax[s_leg][MUS])
                )
                plot_data[s_leg][MUS]["l"] = (
                    temp_mus_len[self.muscles_dict[s_leg][MUS]] - self.muscle_LT[s_leg][MUS]
                ) / self.muscle_L0[s_leg][MUS]
