...
def calculate_kinematic_costs(
    unpacked_angles: np.ndarray, one_step: np.ndarray, n_strides: int, index_vector: np.ndarray
) -> Tuple[float, float]:
    """
    ...
    for i, stride_start_idx in enumerate(stride_indices):
        ...
        for ang_idx in range(num_angles):
            interp_mat[:, ang_idx, i] = np.interp(
                np.linspace(0, stride_data.shape[0], 100), np.arange(stride_data.shape[0]), stride_data[:, ang_idx]
            )
...
def calculate_trunk_cost(unpacked_angles: np.ndarray, trunk_err_type: str, const_theta_tgt: float = 0.0) -> float:
    """
...
def interpolate_gait_cycle(data: np.ndarray, n_points: int = 100) -> np.ndarray:
    """
