...
def create_combined_plot(es: Any, save_path: str, trial_name: str, tracker: OptimizationTracker) -> None:
    ...
    rect = plt.Rectangle(
        (0.65, 0.0), 0.35, 1, facecolor="white", edgecolor="white", transform=fig.transFigure, zorder=2, alpha=1.0
    )
    ...
    try:
        if not tracker.costs_by_gen:  # If no data
            ...
            ax_heatmap.text(0.5, 0.5, "No data available", ha="center", va="center", fontsize=12)
        else:
            ...
            if not np.isnan(heatmap_data).all():
                ...
                boundaries = [
                    ...
                    60,
                    80,
                    100,  # Excellent to Good (greens)
                    130,
                    160,
                    200,  # Moderate (yellows)
                    1000,
                    20000,
                    95000,  # Poor (oranges)
                    96000,
                    ...
                    98000,
                    99000,
                    100000,  # Early Termination (reds)
                    200000,
                    1200000,  # Simulation Error (dark reds)
                ]
                ...
                # Plot heatmap
                im = ax_heatmap.imshow(heatmap_data, aspect="auto", cmap=cmap_costs, norm=norm, interpolation="none")
                ...
                ax_heatmap.text(0.5, 0.5, "No valid cost data available", ha="center", va="center", fontsize=12)

        # Add metadata using CMA-ES values
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        param_text = (
            f"Population Size: {es.popsize}\n"
    ...
    plt.savefig(f"{save_path}/{plot_filename}", dpi=300, bbox_inches="tight")

    pdf_filename = f"{trial_name}_optimization_results.pdf"
    plt.savefig(f"{save_path}/{pdf_filename}", format="pdf", bbox_inches="tight")
    plt.close()
