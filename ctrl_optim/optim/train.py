...
def main():
    ...
    cwd = os.getcwd()
    potential_configs = [
        os.path.join(cwd, "training_configs", f"{config_name}.bat"),  # From current directory (optim/)
        os.path.join(cwd, "..", "training_configs", f"{config_name}.bat"),  # One level up
        os.path.join(
            cwd, "..", "..", "myoassist_reflex", "optim", "training_configs", f"{config_name}.bat"
        ),  # From project root
    ]
    ...
    if input_args.optim_mode == "evaluate":
        ...
        cost_dict = func_Walk_FitCost(
            ...
            input_args.tgt_vel,
            input_args.num_strides,
            input_args.tgt_sym_th,
            input_args.tgt_grf_th,
            env_dict=env_dict,
            cost_print=True,
        )
        ...
        cost_dict_last = func_Walk_FitCost(
            best_last_solution,
            optim_type,
            one_step,
            one_EMG,
