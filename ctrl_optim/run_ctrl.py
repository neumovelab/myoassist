...
def main():
    ...
    PARAMS_FILE_PATH = os.path.join(
        notebook_dir, "results", "optim_results", "tutorial_example", "myorfl_Kine_2D_1_25_2025Aug11_1439_None_BestLast.txt"
    )
    ...
    if LOAD_FROM_FILE:
        ...
        env, config, _ = load_params_and_create_testenv(
            results_dir=results_dir, filename=filename, bat_file_path=config_file_path, sim_time=SIMULATION_TIME
        )
    ...
    for i in range(timesteps):
        ...
        if not env.delayed:
            ...
            frame = env.env.unwrapped.sim.renderer.render_offscreen(camera_id=free_cam, width=video_width, height=video_height)
        else:
            if i % 10 == 0:
