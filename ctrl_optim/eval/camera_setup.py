...
class CameraController:
    ...
    def __init__(
        ...
        # Initialize renderer
        env.env.sim.renderer.setup_renderer(env.env.sim.model.ptr, height=renderer_height, width=renderer_width)

        # Configure visualization flags
        self._setup_visualization_flags(show_contact_forces, show_actuators, scene_options)

        print(f"Camera setup complete. Mode: {config.value}")

    def _setup_visualization_flags(self, show_contact_forces: bool, show_actuators: bool, scene_options: Optional[Dict]):
    ...
    def update_camera(self, timestep: int, dt: float):
        ...
        if self.config == CameraConfig.DEFAULT:
            ...
            if self.rotation_function is not None:
                ...
                rotation_end_timestep = (
                    int((sim_time - self.rotation_early_end) / dt_used)
                    if self.rotation_early_end > 0
                    else int(sim_time / dt_used)
                )
    ...
    def render_frame(self) -> np.ndarray:
        ...
        return self.env.env.sim.renderer.render_offscreen(
            camera_id=self.camera, width=self.renderer_width, height=self.renderer_height
        )
