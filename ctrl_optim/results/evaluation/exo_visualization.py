...
class ExoVisualizer:
    ...
    def create_overlay(
        self,
        env,
        current_torque_r: float,
        current_torque_l: float,
        stance_percent_r: float,
        stance_percent_l: float,
        state_r: str,
        state_l: str,
    ) -> np.ndarray:
        ...
        cv2.putText(
            overlay,
            "EXOSKELETON CONTROL VISUALIZATION",
        ...
        cv2.putText(
            overlay, "RIGHT LEG:", (self.margin, y_pos), self.font, self.font_scale_medium, self.colors["cyan"], self.thickness
        )
        ...
        cv2.putText(
            overlay,
            f"  Stance Phase: {stance_percent_r:.1f}%",
            (self.margin, y_pos),
            self.font,
            self.font_scale_medium,
            self.colors["white"],
        ...
        cv2.putText(
            overlay,
            f"  Applied Torque: {current_torque_r:.2f} Nm",
            (self.margin, y_pos),
            self.font,
            self.font_scale_medium,
            self.colors["yellow"],
        ...
        cv2.putText(
            overlay, "LEFT LEG:", (self.margin, y_pos), self.font, self.font_scale_medium, self.colors["cyan"], self.thickness
        )
        y_pos += self.line_height
        cv2.putText(
            overlay,
        ...
        cv2.putText(
            overlay,
            f"  Stance Phase: {stance_percent_l:.1f}%",
            (self.margin, y_pos),
            self.font,
            self.font_scale_medium,
            self.colors["white"],
        ...
        cv2.putText(
            overlay,
            f"  Applied Torque: {current_torque_l:.2f} Nm",
            (self.margin, y_pos),
            self.font,
            self.font_scale_medium,
            self.colors["yellow"],
    ...
    def _draw_torque_profile(self, overlay: np.ndarray, env, stance_percent_r: float):
        ...
        cv2.rectangle(
            overlay, (profile_x, profile_y), (profile_x + profile_width, profile_y + profile_height), self.colors["white"], 2
        )
        ...
        cv2.putText(
            overlay,
            "0%",
            (profile_x, profile_y + profile_height + 25),
            self.font,
        ...
        cv2.putText(
            overlay,
            "100%",
            (profile_x + profile_width - 35, profile_y + profile_height + 25),
            self.font,
            self.font_scale_small,
            self.colors["white"],
            1,
        ...
        if env.exo_bool and env.ExoCtrl_R is not None:
            ...
            if hasattr(env.ExoCtrl_R, "stance_duration_time") and hasattr(env.ExoCtrl_R, "average_stance_duration"):
                if env.ExoCtrl_R.average_stance_duration > 0:
                    stance_percent_r = np.clip(
                        (env.ExoCtrl_R.stance_duration_time / env.ExoCtrl_R.average_stance_duration) * 100, 0, 100
                    )
            ...
            if hasattr(env.ExoCtrl_L, "stance_duration_time") and hasattr(env.ExoCtrl_L, "average_stance_duration"):
                if env.ExoCtrl_L.average_stance_duration > 0:
                    stance_percent_l = np.clip(
                        (env.ExoCtrl_L.stance_duration_time / env.ExoCtrl_L.average_stance_duration) * 100, 0, 100
                    )
